    ...
    internal sealed class RootfsFeatureService
    {
        ...
        public Task ApplyAtWebAsync(PartitionInfo partition, WorkspaceSession session)
        {
            return Task.Run(delegate
            {
                ...
                var targetBinary = Path.Combine(root, "sbin", "atweb"); var targetHtml = Path.Combine(root, "etc_ro", "web", "at.html"); Directory.CreateDirectory(Path.GetDirectoryName(targetBinary)); Directory.CreateDirectory(Path.GetDirectoryName(targetHtml)); File.Copy(binary, targetBinary, true); File.Copy(html, targetHtml, true); var metadata = new WorkspaceMetadataService(); metadata.Update(root, "/sbin/atweb", new WorkspaceMetadata { Kind = "file", Mode = Convert.ToInt32("775", 8), Owner = "0:0", Modified = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") }); metadata.Update(root, "/etc_ro/web/at.html", new WorkspaceMetadata { Kind = "file", Mode = Convert.ToInt32("775", 8), Owner = "0:0", Modified = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") });
